...
start powershell -NoExit -Command "Set-Location '%TRANSCRIBE_DIR%'; $env:OPENAI_API_KEY=$env:OPENAI_API_KEY; $env:PORT='%PORT%'; python .\app.py"
start powershell -NoExit -Command "Set-Location '%WIDGET_DIR%'; $env:TRANSCRIBE_URL='http://127.0.0.1:%PORT%/upload'; python .\widget.py"

echo Launched transcribe on port %PORT% and widget.
endlocal
